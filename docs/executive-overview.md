...
3. **Discover authors** — Unknown git authors are captured in an author registry and can be assigned to orgs/teams from the TUI or CLI.
4. **Explore interactively** — A keyboard-driven TUI presents four dashboard tabs with drill-down views, pivot modes, and a full trends screen.
...
| Drill-down views | Org → team → individual member drill with ↑↓ arrows |
...
| Time window control | Extend or shrink the visible time window with ←/→ arrows |
...
| Hide unassigned | Toggle visibility of unassigned contributors in contributions view |
...
- **642 tests** across 34 test files — full coverage of collector, aggregator, UI, views, CLI, store, and export/import
...
| `gitradar data` | Export CSV, export/import portable YAML |
